const menuImageFiles: Record<string, string> = {
  ...
  "Chicken Pakoda 8pcs": "chicken Pakoda.webp",
  ...
  "Handi Chicken": "Handi-Chicken.webp",
  ...
  "Handi Chicken 250gm": "Handi-Chicken.webp",
  ...
  "Butter Tandoori Roti": "Butter-Tandori-Roti.webp",
  ...
  "Handi Chicken Combo": "Handi-Chicken.webp",
  "Afghani Chicken Combo": "afghani-chicken-Combo.jpg",
  "Chicken Corma Combo": "Chicken-Korma-combo.webp",
  "Handi Paneer Masala Combo": "handi paneer masala combo.webp",
  ...
  "Noodles Combo (Chilli Paneer)": "Noodles combo(chili Panner).webp",
  "Fried Rice Combo (Chilli Paneer)": "Fried rice combo(Chili Panner).webp",
...
const normalizedMenuImageFiles = Object.fromEntries(
  Object.entries(menuImageFiles).map(([menuItem, fileName]) => [normalizeName(menuItem), fileName]),
);
...
function toImageUrl(fileName: string) {
  ...
  return `${getBaseUrl()}menu-optimized/${encodedFileName}`;
}

function getImageByBackendPath(backendImage = "") {
  ...
  return fileName ? toImageUrl(fileName) : "";
}

function getCategoryFallbackImageUrl(category = "") {
  const normalizedCategory = normalizeName(category);
  if (normalizedCategory.includes("thali")) return toImageUrl("Veg-Thali.jpg");
  if (normalizedCategory.includes("biryani")) return toImageUrl("chicken-handi-biryani.jpg");
  if (normalizedCategory.includes("noodles")) return toImageUrl("Mixed Noddles.webp");
  if (normalizedCategory.includes("rice")) return toImageUrl("Mixed-Fried-Rice.webp");
  if (normalizedCategory.includes("tandoor")) return toImageUrl("Tandoori-Chicken.jpg");
  if (normalizedCategory.includes("roll")) return toImageUrl("chicken Roll.jpg");
  if (normalizedCategory.includes("combo")) return toImageUrl("Dal-Tadka-combo.jpg");
  return toImageUrl("Veg-Thali.jpg");
}

export function getMenuItemImageUrl(itemName = "", category = "", backendImage = "") {
  const mappedFile = menuImageFiles[itemName];
  if (mappedFile) return toImageUrl(mappedFile);
